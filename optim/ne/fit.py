...
import numpy as np
from optim.ne.agent import BaseAgent
from optim.ne.space import BaseSpace
from optim.ne.utils.compute import (
...
from optim.ne.utils.evolve import evaluate_on_cpu, evaluate_on_gpu, mutate
from optim.ne.utils.exchange import (exchange_agents,
                                     update_exchange_and_mutate_info)
from optim.ne.utils.initialize import (initialize_agents,
                                       initialize_common_variables,
                                       initialize_gpu_comm)
from optim.ne.utils.readwrite import (find_existing_save_points, load_state,
                                      save_state)
from optim.ne.utils.type import Generation_results_type
from optim.ne.utils.validate import validate_space
from optim.ne.utils.wandb import setup_wandb, terminate_wandb
...
def evolve(
    ...
    for curr_gen in range(prev_num_gens + 1, config.total_num_gens + 1):
        ...
        if curr_gen == 1:
            # See https://github.com/maximilienleclei/ai_repo/blob/main/docs/genetic.pdf
