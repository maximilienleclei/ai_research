...
class BaseCAM(nn.Module, ABC):
    ...
    previous time step (if :paramref:`teacher_forcing`, otherwise
    zeroes once again).
    """
    ...
    def __init__(
        ...
        if model_partial.func == FNNConfig:
            ...
            self.model: RNN | LSTM = model_partial(                input_size=self.num_input_features,
                batch_first=True,
            )
            self.proj_out = nn.Linear(
                in_features=hidden_size,                out_features=self.num_output_logits,
            )
        else:
            if model_partial.func == MambaConfig:
                self.model: Mamba = Mamba(model_partial())            else:  # model_partial.func == Mamba2Config:
                self.model: Mamba2 = Mamba2(model_partial())
            self.proj_in = nn.Linear(
                in_features=self.num_input_features,
                out_features=self.model.config.d_model,            )
            self.proj_out = nn.Linear(
                in_features=self.model.config.d_model,                out_features=self.num_output_logits,
            )
    ...
    def predict_sequence_features_and_compute_loss(
        ...
        Returns:
            The predicted sequence of features and the loss.
        """
    ...
    def predict_sequence_logits_fitting(
        ...
        :paramref:`conditioning_sequence_features` is
        concatenated with shifted :paramref:`target_sequence_features`
        (if :paramref:`.teacher_forcing`) or a zeroed-out tensor of
        :paramref:`target_sequence_features` 's shape before being
    ...
    def predict_sequence_logits_and_features_inference(
        ...
        Passes :paramref:`conditioning_sequence_features` through the
    ...
    def predict_timestep_logits_inference(
        ...
        :paramref:`previous_predicted_timestep_features_or_zeroes` and
        :paramref:`conditioning_timestep_features` are
        concatenated before being passed through the model.
        """
        ...
        if isinstance(self.model, Mamba | Mamba2):
            ...
            x, self.cache = self.model.step(x, self.cache)            x = rearrange(tensor=x, pattern="BS HS -> BS 1 HS")
    ...
    def reset(
        ...
        """Resets the model's state/cache."""
        self.cache: (
