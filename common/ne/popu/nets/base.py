...
class BaseNets(ABC):
    ...
    Attributes
    ----------
    config : BaseNetsConfig
        Configuration containing device, dimensions, and population size.
    ...
    def __init__(self: "BaseNets", config: BaseNetsConfig) -> None:
        ...
        ----------
        config : BaseNetsConfig
            Network configuration with device and dimension info.
        """
        self.config = config
    ...
    def resample(self: "BaseNets", indices: Tensor) -> None:
        ...
        ----------
        indices : Tensor
            Index tensor of shape (num_nets,) specifying which network
    ...
    def __call__(self: "BaseNets", x: Tensor) -> Tensor:
        ...
        ----------
        x : Tensor
            Input observations with shape (num_nets, input_dim).
