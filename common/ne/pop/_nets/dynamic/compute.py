...
def barebone_run(verbose: bool = True):
    ...
    nets: list[Net] = [
        Net(NUM_INPUTS, NUM_OUTPUTS) for _ in range(POPULATION_SIZE)
    ]
    ...
    for i in range(POPULATION_SIZE):
        ...
        net_in_nodes_indices: Int[Tensor, "NET_NMN 3"] = nets[
            i
        ].in_nodes_indices
    ...
    num_network_passes_per_input: Int[Tensor, "POPULATION_SIZE"] = (
        torch.tensor([net.num_network_passes_per_input for net in nets])
    )
    max_num_network_passes_per_input: int = max(
    ...
    num_network_passes_per_input_mask = (
        num_network_passes_per_input_mask.bool()
    )
    ...
    for i in range(POPULATION_SIZE):
        start = input_nodes_start_indices[i]
            ...
            else input_nodes_start_indices[i + 1]
        )
